...
class ATEExample(object):
    def __init__(self, context, labels, pos_tag, dependency_graph):
        ...
        new_pos = []
        j = 0
        new_depen = np.zeros_like(dependency_graph)
        for i in range(len(labels)):
            if labels[i] == "O":
                new_sent.append(context[temp[j]])
                new_pos.append(pos_tag[temp[j]])
                new_depen[:, temp[j]] = dependency_graph[temp[j]]
                new_depen[temp[j], :] = dependency_graph[temp[j]]
                j += 1
            else:
                new_sent.append(context[i])
                new_pos.append(pos_tag[i])
                new_depen[:, i] = dependency_graph[i]
                new_depen[i, :] = dependency_graph[i]
        self.context = " ".join(new_sent)
        self.labels = labels
        self.pos = new_pos
        self.dependency_graph = new_depen
    ...
    def read_example(self, file_name):
        ...
        with open(file_name, 'r', encoding="utf8") as fin:
            ...
            for line in fin:
                ...
                # pos_tag = pos_tag.split(" ")
                assert len(context.split(" ")) == len(label_str.split(" "))
                assert len(context.split(" ")) == len(pos_tag.split(" "))
                example = ATEExample(context, label_str, pos_tag, idx2graph[context])
