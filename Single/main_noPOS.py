...
from preprocess_noPos import load_dataset
from torch.utils.data import DataLoader
from model_noPos import BertForTokenClassification
...
logging.basicConfig(format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s',
                    level=logging.DEBUG,
                    filename=args.log_path + '/train.log',
                    filemode='w')
...
def train(configs):
    ...
    for epoch in range(1, configs.epochs + 1):
        for step, batch in tqdm(enumerate(train_dataloader), desc="Traning", total=len(train_dataloader)):
            ...
            model.zero_grad()
            inputs = {
                    "input_ids": batch["input_ids"][:, :args.max_seq_length].to(device),
                    "attention_mask": batch["attention_mask"][:, :args.max_seq_length].to(device),
                    "token_type_ids": batch["token_type_ids"][:, :args.max_seq_length].to(device),
                    "dependency_graph": batch["dependency_graph"].to(device),
    ...
    return dev_dataloader, test_dataloader, tokenizer, output_file
...
def valid(model, dev_dataloader, device, tokenizer=None):
    ...
    for step, batch in tqdm(enumerate(dev_dataloader), desc="Validing...", total=len(dev_dataloader)):
        inputs = {
                    "input_ids": batch["input_ids"][:, :args.max_seq_length].to(device), # 0
                    "attention_mask": batch["attention_mask"][:, :args.max_seq_length].to(device), # 1
                    "token_type_ids": batch["token_type_ids"][:, :args.max_seq_length].to(device), # 2
                    "dependency_graph": batch["dependency_graph"].to(device),
                }
        labels = batch["labels"]
        logits = model(**inputs)
...
if __name__ == "__main__":
    ...
    dev_dataloader, test_dataloader, tokenizer, output_file = train(args)
    ...
    model.to(device1)
    p, r, f = valid(model, test_dataloader, device1)
    print("***************************************************************************************************")
    print("Precision：", p, "Recall", r, "F1:", f)
    print("***************************************************************************************************")
    fitlog.add_best_metric({"Test": {"Precision": p, "Recall": r, "F1": f}})

    p, r, f = valid(model, dev_dataloader, device1, tokenizer)
    fitlog.finish() 